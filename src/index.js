...
export default (to, from, next,store,confg = {})=>{
    let confgs = _.merge({
        //登录数据模块名
        moduleName:"login",
        //布局模块名
    ...
    //登录权限判断
    if (store.state.airforce[confgs.moduleName] && confgs.isLogin(store.state.airforce[confgs.moduleName])){//判断本地有无登陆数据或登陆code是否为20000
        //已登录，直接进
        next();
    }else{
